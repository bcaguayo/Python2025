...
IP_SHEETNAME = 'Sheet1'
OP_FILENAME = 'Blackhorse.xlsx'
OP_SHEETNAME = 'SUBMISSION'
...
mappings = [['B', 'a', Copy.CIA], ['A', 'b', Copy.VALUE], ['E', 'c', Copy.VALUE], ['F', 'e', Copy.VALUE], 
            ['G', 'f', Copy.VALUE], ['H', 'g', Copy.VALUE], ['I', 'h', Copy.DATE], 
            ['J', 'o', Copy.VALUE], ['K', 'p', Copy.VALUE], ['L', 'q', Copy.VALUE], ['M', 'r', Copy.VALUE]]
...
for row in range(MIN_ROW, MAX_ROW):
    ...
    fan = wsA.cell(row=row, column=5).value
    if fan in seen:
        # Print Dupes
        print(fan)
        continue
